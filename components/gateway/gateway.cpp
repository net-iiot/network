...
namespace NetworkMesh
{
    ...
    static void init_wifi()
    {
        ...
        ssid = CONFIG_NETWORK_GATEWAY_WIFI_SSID;
        ESP_LOGI(TAG, "CONFIG_NETWORK_GATEWAY_WIFI_SSID = '%s'", ssid);
        ESP_LOGI(TAG, "   Tamanho: %zu bytes", strlen(ssid));
#else
        ESP_LOGW(TAG, "CONFIG_NETWORK_GATEWAY_WIFI_SSID não definido, usando padrão");
        ssid = "NetworkGateway";
#endif

#ifdef CONFIG_NETWORK_GATEWAY_WIFI_PASSWORD
        password = CONFIG_NETWORK_GATEWAY_WIFI_PASSWORD;
        ESP_LOGI(TAG, "CONFIG_NETWORK_GATEWAY_WIFI_PASSWORD = %s",
...
#else
        ESP_LOGW(TAG, "CONFIG_NETWORK_GATEWAY_WIFI_PASSWORD não definido, rede aberta");
        password = "";
#endif
    ...
    void Gateway::init(const std::string &server_url)
    {
        ...
        else
        {
            ...
            s_server_url = CONFIG_NETWORK_GATEWAY_SERVER_URL;
#else
            s_server_url = "http://192.168.1.100:8080";
#endif
        }
    ...
    void Gateway::uart_listen_task(void *)
    {
        ...
        const TickType_t status_log_interval = pdMS_TO_TICKS(10000);
        ...
        for (;;)
        {
            ...
            if (pkt.type == Protocol::PacketType::EVENT)
            {
                ...
                else if (pkt.method == "OTA_RESULT")
                {
                    ...
                    cJSON *ota_root = cJSON_Parse(pkt.body.c_str());
                    if (ota_root)
                    {
                        cJSON *jcmd = cJSON_GetObjectItem(ota_root, "command_id");
                        if (jcmd && cJSON_IsString(jcmd))
                            command_id = jcmd->valuestring;

                        cJSON *jnode = cJSON_GetObjectItem(ota_root, "node_id");
                        if (jnode && cJSON_IsString(jnode))
                            node_id = jnode->valuestring;

                        cJSON *jerr = cJSON_GetObjectItem(ota_root, "error");
                        if (jerr && cJSON_IsString(jerr))
                            error_msg = jerr->valuestring;

                        cJSON *jsuccess = cJSON_GetObjectItem(ota_root, "success");
                        if (jsuccess)
                            success = cJSON_IsTrue(jsuccess);

                        cJSON_Delete(ota_root);
                    }

                    if (node_id.empty())
                        node_id = pkt.route.src;
...
#ifdef CONFIG_NETWORK_GATEWAY_WIFI_SSID
        target_ssid = CONFIG_NETWORK_GATEWAY_WIFI_SSID;
#endif
        ESP_LOGI(TAG, "═══════════════════════════════════════════════════════");
    ...
    void Gateway::http_request_task(void *)
    {
        for (;;)
        {
            ...
            HttpContextManager::instance().cleanup_old_contexts(300000);
        }
    }
    ...
    void Gateway::polling_task(void *)
    {
        ...
        const uint32_t map_send_interval_ms = 300000;
        const uint32_t ota_check_interval_ms = 30000;
